...
| **TW-08 团队动态与通知（批次 3 第三部分）** | ✅ **后端已交付**（3-T7，两轮评审已修）：迁移 0026、事件写入点、动态/通知读取路由、`test:m5:activity:integration` 21 用例；前端（3-T8）未开始（已拆分） | `24774e6`（CI `34666778699` ✅） |
| TW-09 | ⬜ 未开始 | — |
...
- 评审：规格符合性「有条件符合 → 已修」；质量评审「PASS，一处 P1 必修」；两轮发现已全部修复并各自反证。交付：`24774e6` 已合入 `main`，CI `M6 quality gate` 通过（`34666778699`）。待做：前端（3-T8）右栏「最近动态」摘要 + 「查看全部」抽屉 + 未读与静音入口。
