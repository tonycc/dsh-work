...
**文档性质：** 历史交互基线；当前实现状态见 [MVP 路线图与交付状态](../project/mvp-roadmap.md)
...
| Agent 管理 | `/agents` | 三步创建、配置确认、服务端校验、发布、停用、版本和回滚 |
| Skill 与工具 | `/capabilities` | Skill 管理；预置 Tool/Connector 查看、权限、启停和健康 |
| Session 列表 | `/sessions` | 治理元数据，不默认展示消息正文 |
| 模型用量 | `/model-usage` | Token、时延、状态和 Trace |
| 工作空间 | `/workspaces` | 团队工作空间与真实用量只读查看 |
| 工具权限 | `/permissions` | Tool 授权角色、数据范围和审批策略 |
...
- 系统为每位用户维护唯一默认个人工作空间，同时保留团队工作空间；
- 所有对话、文件和成果必须归属个人或团队空间；
- 团队工作空间创建后由创建人自动成为首个成员；
...
M0 原型确认的是页面、信息架构、权限表现和交互；在 2026-08-29 确认时，它不代表以下能力已经实现：
...
其中多项能力已在后续里程碑落地。这里保留原始基线语义，不作为现状清单；当前完成项和开放 Gate 统一查阅 [MVP 路线图与交付状态](../project/mvp-roadmap.md)。
...
任何实施任务修改已确认交互时，必须同步更新本基线、[架构总览](../architecture/overview.md)、相关 UI 契约和验收用例。
