...
| 取消和超时可靠终止真实运行 | 部分满足 | Mock ACP 通过；真实模型配置已确认但凭据未配置，真实 Tool 仍待 D-05 |
| 一个批准模型真实运行并记录 Token | 未满足 | D-02 已确认继承 DSH 默认配置；当前缺少 `DEEPSEEK_API_KEY`，且 ACP Token observer 尚待实现 |
