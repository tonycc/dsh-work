...
| A-11 | DSH 作为受管 Runtime 制品独立交付；不把 DSH 源码并入 dsh-work | 已替代 | 项目 Owner | 由 A-15 替代；确认上游没有该模式，原内部制品契约尚无构建和发布流水线 |
...
| A-15 | DSH 使用独立源码 checkout；生产固定 `0.1.2-rc.1`，本地开发显式兼容 `0.1.1-rc.2`，两者均固定 Commit；内部离线制品需另行实现 | 已确认 | 项目 Owner | 旧 Adapter 仅限开发，生产预检拒绝兼容开关；不把 DSH 源码并入 dsh-work |

## 2. 启动外部决策

| 编号 | 决策事项 | 当前结论/候选 | 状态 | 责任角色 | 截止时间 | 阻塞影响 |
|---|---|---|---|---|---|---|
| D-01 | DSH Runtime 版本和入口 | 生产使用 `0.1.2-rc.1` / `76fda729…` 及正式 `acp` profile；本地开发可显式选择 `0.1.1-rc.2` / `b150a551…` 的 legacy ACP Adapter | 工程入口已确认，生产交付待确定 | 项目 Owner | M5 部署前 | 两个版本均精确校验并预检；兼容模式禁止用于生产；新版本真实模型/Tool/UAT 未通过前不得宣称试点就绪 |
...
| D-08 | Mac mini 环境 | 服务账号 `deploy`、固定 IP `192.168.33.20` 已确认；目录、证书、网络准入、异机备份挂载和 UPS 仍须现场核验 | 部分确认 | 项目 Owner | M5 部署前 | 未确认项继续阻塞预生产部署，不阻塞 M0 |
...
- 交付规范见 `docs/deployment/dsh-runtime-delivery.md`；当前经验证的入口是独立源码 checkout。上游没有 `bin/dsh-acp + dsh-runtime.json` 受管制品模式，正式 npm 或离线制品交付完成前不得宣称 staging/pilot 已具备该能力。
